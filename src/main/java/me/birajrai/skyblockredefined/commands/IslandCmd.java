...
import me.birajrai.skyblockredefined.events.*;
import me.birajrai.skyblockredefined.listeners.PlayerEvents;
import me.birajrai.skyblockredefined.panels.ControlPanel;
import me.birajrai.skyblockredefined.schematics.Schematic;
import me.birajrai.skyblockredefined.schematics.Schematic.PasteReason;
...
public class IslandCmd implements CommandExecutor, TabCompleter {
     ...
     * Constructor
     *
     * @param skyBlockRedefined
     */
    public IslandCmd(SkyBlockRedefined skyBlockRedefined) {
        // Plugin instance
    ...
    public void loadSchematics() {
        ...
        if (schematics.containsKey("default")) {
            ...
            schematics.get("default").setName("Normal Mode");
            schematics.get("default").setDescription("A SkyBlock Island");
    ...
    public void newIsland(final Player player, final Schematic schematic) {
        ...
        if (schematic != null) {
            ...
            if (Settings.createNether && schematic.isInNether() && Settings.newNether && SkyBlockRedefined.getNetherWorld() != null) {
                // Nether start
                ...
                next = next.toVector().toLocation(SkyBlockRedefined.getNetherWorld());
                // Set the player's island location to this new spot
                plugin.getPlayers().setIslandLocation(playerUUID, next);
                ...
                if (Settings.createNether && Settings.newNether && SkyBlockRedefined.getNetherWorld() != null) {
                    ...
                    final Location netherLoc = next.toVector().toLocation(SkyBlockRedefined.getNetherWorld());
    ...
    private Location getNextIsland(UUID playerUUID) {
        ...
        if (last == null) {
            last = new Location(SkyBlockRedefined.getIslandWorld(), Settings.islandXOffset + Settings.islandStartX, Settings.islandHeight, Settings.islandZOffset + Settings.islandStartZ);
        }
...
//                    Util.sendMessage(player, ChatColor.GOLD + "This plugin is custom made for the server Wart SkyBlock");
//                    return true;
                    // Spawn enderman
            ...
            if (split[0].equalsIgnoreCase("minishop") || split[0].equalsIgnoreCase("ms")) {
                if (Settings.useEconomy && Settings.useMinishop) {
                    ...
                    if (player.getWorld().equals(SkyBlockRedefined.getIslandWorld()) || player.getWorld().equals(SkyBlockRedefined.getNetherWorld())) {
            ...
            if (split[0].equalsIgnoreCase("warp")) {
                ...
                if (VaultHelper.checkPerm(player, Settings.PERMPREFIX + "island.spawn")) {
                    // go to spawn
                    Location l = SkyBlockRedefined.getIslandWorld().getSpawnLocation();
                ...
                // Leave team command
                if (VaultHelper.checkPerm(player, Settings.PERMPREFIX + "team.join")) {
                    if (player.getWorld().equals(SkyBlockRedefined.getIslandWorld()) ||
                            (Settings.createNether && Settings.newNether &&
                                    SkyBlockRedefined.getNetherWorld() != null && player.getWorld().equals(SkyBlockRedefined.getNetherWorld()))) {
                        if (plugin.getPlayers().inTeam(playerUUID)) {
                        ...
                        if (VaultHelper.checkPerm(player, Settings.PERMPREFIX + "island.lang")) {
                            if (!NumberUtils.isDigits(split[1])) {
                                ...
                                try {
                                    ...
                                    for (Locale locale : plugin.getAvailableLocales().values()) {
                            ...
                            if (VaultHelper.checkPerm(player, Settings.PERMPREFIX + "island.warp")) {
                                ...
                                if (warpList.isEmpty()) {
                                    ...
                                    if (foundWarp == null) {
                                        ...
                                        boolean pvp = false;
                                        if (island != null && ((warpSpot.getWorld().equals(SkyBlockRedefined.getIslandWorld()) && island.getIgsFlag(SettingsFlag.PVP))
                                                || (warpSpot.getWorld().equals(SkyBlockRedefined.getNetherWorld()) && island.getIgsFlag(SettingsFlag.NETHER_PVP)))) {
                                            pvp = true;
                                        }
                            ...
                            if (VaultHelper.checkPerm(player, Settings.PERMPREFIX + "team.kick")) {
                                ...
                                if (teamMembers.contains(targetPlayer)) {
                                    ...
                                    if (target != null) {
                                        ...
                                        // leader
                                        if (target.getWorld().equals(SkyBlockRedefined.getIslandWorld())) {
                                        ...
                                        if (!target.performCommand(Settings.SPAWNCOMMAND)) {
                                            target.teleport(SkyBlockRedefined.getIslandWorld().getSpawnLocation());
                                        }
                                    } else {
                                        // Offline
                                        if (DEBUG)
