package me.birajrai.skyblockredefined.commands;

import com.google.common.collect.HashMultiset;
import com.google.common.collect.Multiset;
import me.birajrai.skyblockredefined.*;
import me.birajrai.skyblockredefined.Island.SettingsFlag;
import me.birajrai.skyblockredefined.Locale;
import me.birajrai.skyblockredefined.Settings.GameType;
import me.birajrai.skyblockredefined.listeners.LavaCheck;
import me.birajrai.skyblockredefined.panels.ControlPanel;
import me.birajrai.skyblockredefined.panels.SetBiome;
import me.birajrai.skyblockredefined.util.Util;
import me.birajrai.skyblockredefined.util.VaultHelper;
import me.birajrai.skyblockredefined.util.teleport.SafeTeleportBuilder;
import org.bukkit.*;
...
public class AdminCmd implements CommandExecutor, TabCompleter {
    ...
    public AdminCmd(SkyBlockRedefined skyBlockRedefined) {
    ...
    private void help(CommandSender sender, String label) {
        if (!(sender instanceof Player)) {
            ...
            if (Settings.createNether && Settings.newNether && SkyBlockRedefined.getNetherWorld() != null && (VaultHelper.checkPerm(player, Settings.PERMPREFIX + "mod.tpnether") || player.isOp())) {
                ...
                if (split[0].equalsIgnoreCase("settingsreset")) {
                    ...
                    if (Settings.teamChat && split[0].equalsIgnoreCase("spy")) {
                        ...
                        if (split[0].equalsIgnoreCase("topbreeders")) {
                            ...
                            final List<Entity> allEntities = SkyBlockRedefined.getIslandWorld().getEntities();
                            final World islandWorld = SkyBlockRedefined.getIslandWorld();
                            final World netherWorld = SkyBlockRedefined.getNetherWorld();
        ...
        case 2:
                ...
                if (plugin.getAvailableLocales().keySet().contains(split[1])) {
                    ...
                    try {
                        ...
                        for (String code: fl.list()) {
                            ...
                            availableLocales.put(code, new Locale(plugin, code, index++));
                        }
                    ...
                    if (!availableLocales.containsKey(Settings.defaultLanguage)) {
                        ...
                        availableLocales.put(Settings.defaultLanguage, new Locale(plugin, Settings.defaultLanguage, 0));
                    }
            ...
            if (split[0].equalsIgnoreCase("purge")) {
                ...
                if (split[1].equalsIgnoreCase("allow") || split[1].equalsIgnoreCase("disallow")) {
                    ...
                    Player p = (Player) sender;
                    // Island spawn must be in the island world
                    if (!p.getLocation().getWorld().equals(SkyBlockRedefined.getIslandWorld()) && !p.getLocation().getWorld().equals(SkyBlockRedefined.getNetherWorld())) {
                ...
                if (!plugin.getPlayers().isAKnownPlayer(targetUUID)) {
                    ...
                    if (plugin.getPlayers().hasIsland(targetUUID) || plugin.getPlayers().inTeam(targetUUID)) {
                        ...
                        Location warpSpot = plugin.getPlayers().getIslandLocation(targetUUID).toVector().toLocation(SkyBlockRedefined.getIslandWorld());
                        ...
                        plugin.getGrid();
                        if (home != null && home.getWorld().equals(SkyBlockRedefined.getIslandWorld()) && GridManager.isSafeLocation(home)) {
                ...
                if (!plugin.getPlayers().isAKnownPlayer(targetUUID)) {
                    ...
                    if (plugin.getPlayers().hasIsland(targetUUID) || plugin.getPlayers().inTeam(targetUUID)) {
                        ...
                        Location warpSpot = plugin.getPlayers().getIslandLocation(targetUUID).toVector().toLocation(SkyBlockRedefined.getNetherWorld());
                        String failureMessage = ChatColor.RED + plugin.myLocale(player.getUniqueId()).adminTpManualWarp.replace("[location]", warpSpot.getBlockX() + " " + warpSpot.getBlockY() + " "
                                + warpSpot.getBlockZ());
                        // Try the player's home first
                        Location home = plugin.getPlayers().getHomeLocation(targetUUID);
                        plugin.getGrid();
                        if (home != null && home.getWorld().equals(SkyBlockRedefined.getNetherWorld()) && GridManager.isSafeLocation(home)) {
                            player.teleport(home);
                            return true;
                        }
                        new SafeTeleportBuilder(plugin)
    ...
    public boolean adminSetPlayerIsland(final CommandSender sender, final Location l, final UUID newOwner) {
        ...
        if (sender instanceof Player && Settings.createNether && Settings.newNether && 
                ((Player)sender).getWorld().equals(SkyBlockRedefined.getNetherWorld())) {
            // Island in new nether
            plugin.getPlayers().setHomeLocation(newOwner, island.getCenter().toVector().toLocation(SkyBlockRedefined.getNetherWorld()));
            plugin.getPlayers().setIslandLocation(newOwner, island.getCenter().toVector().toLocation(SkyBlockRedefined.getNetherWorld()));
        } else {
            // Island in overworld
