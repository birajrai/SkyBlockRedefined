...
public class Updater {
    ...
    public boolean shouldUpdate(String localVersion, String remoteVersion) {
        //plugin.getLogger().info("DEBUG: localVersion = " + localVersion + " remoteVersion = " + remoteVersion);
        // Check to see if the latest file is newer that this one
        // Need to escape the period in the regex expression
        String[] updateVer = remoteVersion.split("\\.");
        //plugin.getLogger().info("DEBUG: split length = " + updateVer.length);
        // Check the version #'s
        String[] pluginVer = localVersion.split("\\.");
        ...
        for (int i = 0; i < Math.max(updateVer.length, pluginVer.length); i++) {
            try {
                ...
                //plugin.getLogger().info("DEBUG: update is " + updateCheck + " plugin is " + pluginCheck);
                if (updateCheck < pluginCheck) {
                    //plugin.getLogger().info("DEBUG: plugin is newer!");
                    //plugin is newer
                    return false;
                } else if (updateCheck > pluginCheck) {
                    //plugin.getLogger().info("DEBUG: update is newer!");
                    return true;
                }
            } catch (Exception e) {
                plugin.getLogger().warning("Could not determine update's version # ");
                plugin.getLogger().warning("Plugin version: "+ localVersion);
                plugin.getLogger().warning("Update version: " + remoteVersion);
            }
        }
        return false;
    }
