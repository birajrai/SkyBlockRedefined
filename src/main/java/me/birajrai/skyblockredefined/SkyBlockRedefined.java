...
import me.birajrai.skyblockredefined.NotSetup.Reason;
import me.birajrai.skyblockredefined.Settings.GameType;
import me.birajrai.skyblockredefined.Updater.UpdateResult;
import me.birajrai.skyblockredefined.Updater.UpdateType;
import me.birajrai.skyblockredefined.commands.AdminCmd;
import me.birajrai.skyblockredefined.commands.Challenges;
import me.birajrai.skyblockredefined.commands.IslandCmd;
import me.birajrai.skyblockredefined.events.IslandDeleteEvent;
import me.birajrai.skyblockredefined.events.IslandPreDeleteEvent;
import me.birajrai.skyblockredefined.events.ReadyEvent;
import me.birajrai.skyblockredefined.generators.ChunkGeneratorWorld;
import me.birajrai.skyblockredefined.listeners.*;
import me.birajrai.skyblockredefined.panels.*;
import me.birajrai.skyblockredefined.util.HeadGetter;
 ...
 */
public class SkyBlockRedefined extends JavaPlugin {
    private static final boolean DEBUG = false;
    // This plugin
    private static SkyBlockRedefined plugin;
    // The ASkyBlock world
        ...
        if (islandWorld == null) {
            ...
            if (!Settings.useOwnGenerator && Bukkit.getServer().getPluginManager().isPluginEnabled("Multiverse-Core")) {
                ...
                if (!Bukkit.isPrimaryThread()) {
                    Bukkit.getScheduler().runTask(plugin, SkyBlockRedefined::registerMultiverse);
                } else {
                    registerMultiverse();
                }
            }
     ...
     */
    public static SkyBlockRedefined getPlugin() {
        return plugin;
    }
        ...
        getServer().getScheduler().runTask(this, new Runnable() {
            ...
            public void run() {
                ...
                if (!Settings.useOwnGenerator && getServer().getWorld(Settings.worldName).getGenerator() == null) {
                    ...
                    getLogger().severe("********* The Generator for " + SkyBlockRedefined.this.getName() + " is not registered so the plugin cannot start ********");
                    ...
                    getLogger().severe("    generator: " + SkyBlockRedefined.this.getName());
                    if (Settings.GAMETYPE.equals(Settings.GameType.ASKYBLOCK)) {
                    ...
                    HandlerList.unregisterAll(SkyBlockRedefined.this);
                    return;
                }
                ...
                getServer().getScheduler().runTask(SkyBlockRedefined.this, () -> {
                    // load the list - order matters - grid first, then top
                    // ten to optimize upgrades
                    // Load grid
                    if (grid == null) {
                        grid = new GridManager(SkyBlockRedefined.this);
                    }
                    // Register events
                    registerEvents();
                    ...
                    // Load TinyDb
                    if (tinyDB == null) {
                        tinyDB = new TinyDB(SkyBlockRedefined.this);
                    }
                    // Run head getter
                    ...
                    if (Settings.useWarpPanel) {
                        ...
                        warpPanel = new WarpPanel(SkyBlockRedefined.this);
                        getServer().getPluginManager().registerEvents(warpPanel, SkyBlockRedefined.this);
                    }
                    topTen = new TopTen(SkyBlockRedefined.this);
                    // Load the TopTen GUI
                    if (!Settings.displayIslandTopTenInChat){
                        getServer().getPluginManager().registerEvents(topTen, SkyBlockRedefined.this);
                    }
                    // Minishop - must wait for economy to load before we can use
                    // econ
                    getServer().getPluginManager().registerEvents(new ControlPanel(SkyBlockRedefined.this), SkyBlockRedefined.this);
                    // Settings
                    settingsPanel = new SettingsPanel(SkyBlockRedefined.this);
                    getServer().getPluginManager().registerEvents(settingsPanel, SkyBlockRedefined.this);
                    // Biomes
                    // Load Biomes
                    biomes = new BiomesPanel(SkyBlockRedefined.this);
                    getServer().getPluginManager().registerEvents(biomes, SkyBlockRedefined.this);

                    // Add any online players to the DB
                    for (Player onlinePlayer : SkyBlockRedefined.this.getServer().getOnlinePlayers()) {
                        tinyDB.savePlayerName(onlinePlayer.getName(), onlinePlayer.getUniqueId());
                    }
                    if (Settings.backupDuration > 0) {
                        new AsyncBackup(SkyBlockRedefined.this);
                    }
                    // Load the coops
                ...
                if (Settings.updateCheck) {
                    ...
                    new BukkitRunnable() {
                        ...
                        public void run() {
                            if (count++ > 20) {
                                SkyBlockRedefined.this.getLogger().info("No updates found. (No response from server after 20s)");
                                this.cancel();
                            } else {
                                ...
                                if (updateCheck != null) {
                                    ...
                                    case DISABLED:
                                        SkyBlockRedefined.this.getLogger().info("Updating has been disabled");
                                        break;
                                    case FAIL_APIKEY:
                                        SkyBlockRedefined.this.getLogger().info("API key failed");
                                        break;
                                    case FAIL_BADID:
                                        SkyBlockRedefined.this.getLogger().info("Bad ID");
                                        break;
                                    case FAIL_DBO:
                                        SkyBlockRedefined.this.getLogger().info("Could not connect to updating service");
                                        break;
                                    case FAIL_DOWNLOAD:
                                        SkyBlockRedefined.this.getLogger().info("Downloading failed");
                                        break;
                                    case FAIL_NOVERSION:
                                        SkyBlockRedefined.this.getLogger().info("Could not recognize version");
                                        break;
                                    case NO_UPDATE:
                                        SkyBlockRedefined.this.getLogger().info("No update available.");
                                        break;
                                    case SUCCESS:
                                        SkyBlockRedefined.this.getLogger().info("Success!");
                                        break;
                                    case UPDATE_AVAILABLE:
                                        SkyBlockRedefined.this.getLogger().info("Update available " + updateCheck.getLatestName());
                                        break;
                                    default:
                                        break;
                    ...
                    }.runTaskTimer(SkyBlockRedefined.this, 0L, 20L); // Check status every second
                }

                // Run acid tasks
                acidTask = new AcidTask(SkyBlockRedefined.this);
        ...
        getServer().getScheduler().runTaskAsynchronously(this, new Runnable() {
            ...
            public void run() {
                if (Settings.GAMETYPE.equals(GameType.ASKYBLOCK)) {
                    updateCheck = new Updater(SkyBlockRedefined.this, 85189, SkyBlockRedefined.this.getFile(), UpdateType.NO_DOWNLOAD, true); // ASkyBlock
                } else {
                    updateCheck = new Updater(SkyBlockRedefined.this, 80095, SkyBlockRedefined.this.getFile(), UpdateType.NO_DOWNLOAD, true); // AcidIsland
                }                
            }
        });
    }
     ...
     */
    public void setAvailableLocales(HashMap<String, Locale> availableLocales) {
