...
public class IPItem {
    ...
    private void createToggleableItem(boolean flagValue, Material material, int durability, String name, UUID uuid) {
        ...
        if (flagValue) {
            if (uuid == null)
                description.add(ChatColor.GREEN + SkyBlockRedefined.getPlugin().myLocale().igsAllowed);
            else
                description.add(ChatColor.GREEN + SkyBlockRedefined.getPlugin().myLocale(uuid).igsAllowed);
        } else {
            if (uuid == null)
                description.add(ChatColor.RED + SkyBlockRedefined.getPlugin().myLocale().igsDisallowed);
            else
                description.add(ChatColor.RED + SkyBlockRedefined.getPlugin().myLocale(uuid).igsDisallowed);
        }
