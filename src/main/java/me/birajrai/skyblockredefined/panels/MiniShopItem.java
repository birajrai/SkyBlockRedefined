...
import me.birajrai.skyblockredefined.util.SpawnEgg1_9;
    ...
    public MiniShopItem(Material material, String extra, int slot, String description, int quantity, Double price, Double sellPrice) {
        ...
        try {
           ...
           // Deal with extras
            ...
            if (price > 0D) {
                buyAndSell.add(SkyBlockRedefined.getPlugin().myLocale().minishopBuy + " " + quantity + " @ " + VaultHelper.econ.format(price));
            }
            if (sellPrice > 0D) {
                buyAndSell.add(SkyBlockRedefined.getPlugin().myLocale().minishopSell + " " + quantity + " @ " + VaultHelper.econ.format(sellPrice));
            }
            if (price < 0D && sellPrice < 0D) {
                buyAndSell.add(SkyBlockRedefined.getPlugin().myLocale().minishopOutOfStock);
            }
            meta.setLore(buyAndSell);
            item.setItemMeta(meta);
 
        } catch (Exception ex) {
            SkyBlockRedefined.getPlugin().getLogger().severe("Problem parsing shop item from minishop.yml so skipping it: " + material);
            SkyBlockRedefined.getPlugin().getLogger().severe("Error is : " + ex.getMessage());
            ex.printStackTrace();
            SkyBlockRedefined.getPlugin().getLogger().info("Potential potion types are: ");
            for (PotionType c : PotionType.values())
                SkyBlockRedefined.getPlugin().getLogger().info(c.name());
            SkyBlockRedefined.getPlugin().getLogger().info("Potions can also be EXTENDED, SPLASH or EXTENDEDSPLASH, example WATER_BREATHING:EXTENDED");
        }
