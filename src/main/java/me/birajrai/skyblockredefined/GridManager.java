...
import me.birajrai.skyblockredefined.events.IslandChangeOwnerEvent;
import me.birajrai.skyblockredefined.events.IslandPreTeleportEvent;
...
public class GridManager {
     ...
     */
    public GridManager(SkyBlockRedefined plugin) {
        this.plugin = plugin;
        loadGrid();
    }
            ...
            try {
                ...
                if (islandYaml.contains(Settings.worldName)) {
                    ...
                    if (islandYaml.contains("spawn")) {
                        ...
                        // Validate entries
                        if (spawnLoc != null && spawnLoc.getWorld() != null && spawnLoc.getWorld().equals(SkyBlockRedefined.getIslandWorld())) {
    ...
    protected boolean inWorld(Location loc) {
        if (loc.getWorld().equals(SkyBlockRedefined.getIslandWorld())) {
            return true;
        }
        return Settings.createNether && Settings.newNether && SkyBlockRedefined.getNetherWorld() != null
                && loc.getWorld().equals(SkyBlockRedefined.getNetherWorld());
    }
    ...
    private Location netherIsland(Location islandLocation) {
        ...
        return islandLocation.toVector().toLocation(SkyBlockRedefined.getNetherWorld());
    }
    ...
    public boolean isOnIsland(final Player owner, final Player target) {
        // Check world
        if (target.getWorld().equals(SkyBlockRedefined.getIslandWorld()) || (Settings.newNether && Settings.createNether && target.getWorld().equals(SkyBlockRedefined.getNetherWorld()))) {
        ...
        for (Player player : plugin.getServer().getOnlinePlayers()) {
            if (island.inIslandSpace(player.getLocation())) {
                ...
                if (!player.getUniqueId().equals(uuid) && (plugin.getPlayers().hasIsland(player.getUniqueId()) || plugin.getPlayers().inTeam(player.getUniqueId()))) {
                    ...
                    if (spawn != null) {
                        // go to island spawn
                        player.teleport(SkyBlockRedefined.getIslandWorld().getSpawnLocation());
    ...
    public void setSpawnPoint(Location location) {
        SkyBlockRedefined.getIslandWorld().setSpawnLocation(location.getBlockX(), location.getBlockY(), location.getBlockZ());
